...
function pageselectCallback(pageIndex,jq) {
   ...
   // var pageNum = pageIndex + 1;
    ...
    // 跳转页面
    // window.location.href = "customer/query/for/search.html?pageNum="+pageNum;
    // 跳转页面，带查询条件的页面跳转    pageIndex从0开始，pageNum从1开始
   // window.location.href = "customer/query/for/search.html?pageNum=" + pageNum + "&keyword=${param.keyword}";
    window.location.href = "customer/query/for/search?pageNum="+(pageIndex + 1)+"&keyword="+window.keyword;
    return false;
}


// 封装执行批量删除的函数
function doBatchRemove(customerIdArray) {
    ...
    // 发送Ajax请求执行批量删除
    var requestBody = JSON.stringify(customerIdArray);

    // 发送Ajax请求将customerIdArray发送给handler方法
    $.ajax({
        "url": "customer/batch/remove.json",	// 服务器端接收请求的URL地址
        ...
        "success": function (response) {		// 服务器处理请求成功后执行的函数，响应体以参数形式传入当前函数
            ...
            if (result == "SUCCESS") {
                // 跳转页面
                window.location.href = "customer/query/for/search?pageNum="+window.pageNum+"&keyword="+window.keyword;
            }
